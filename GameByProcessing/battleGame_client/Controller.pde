...
void keyReleased() {
    ...
    if (key == 'd') player2.speed.x = 1 + player2.speed.x/10.0;

    if (key == 'a')player2.speed.x = -1 + player2.speed.x/10.0;
   
    if (key == ' ') {
        C.lastATK = C.now;
        player2.alpha = 0;
        player2.ATKpos = player2.pos.copy();    
        player2.isATK = true;  
        player2.ATKAng = player2.faceAng;
    }
}
